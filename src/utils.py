...
def setup_experiment(args, *stats):
    ...
    logger.info(f"实验将存储在 {log_dir.resolve()}\n")
    logger.info("")
    if args.exp_name == "":
...
def setup_evaluation(args, *stats):
    ...
    args_dict = load_args(args.load_path_stage_B)
    if isinstance(args_dict, dict):
        logger.info("\n".join(f"{k}: {str(v)}" for k, v in sorted(args_dict.items())))
    elif isinstance(args_dict, Namespace):
        args_dict = vars(args_dict)
        logger.info("\n".join(f"{k}: {str(v)}" for k, v in sorted(args_dict.items())))
    else:
        logger.warning(f"Returned object is not a dictionary or Namespace, but {type(args_dict)}")
...
def load_args(path):
    ...
    class WindowsCompatibleUnpickler(pickle.Unpickler):
    ...
    with open(path / "args.pkl", 'rb') as f:
        return WindowsCompatibleUnpickler(f).load()
...
def get_best_evaluation(dataset, targets, a_p, v_p, t_p, mode, device, distance_fn, best_beta=None, save_performances=False, args=None, attention_weights=None):
    ...
    steps = (end - start) * 5 + 1
    betas = torch.tensor([best_beta], dtype=torch.float, device=device) if best_beta else torch.linspace(start, end, steps, device=device)
    ...
    seen_unseen_array = torch.tensor(np.sort(np.concatenate((dataset.seen_class_ids, dataset.unseen_class_ids))), dtype=torch.long, device=device)

    classes_embeddings = t_p
    with torch.no_grad():
        for beta in betas:
            if a_p is None:
                distance_mat = torch.zeros((v_p.shape[0], len(dataset.all_class_ids)), dtype=torch.float, device=device) + 99999999999999
                distance_mat_zsl = torch.zeros((v_p.shape[0], len(dataset.all_class_ids)), dtype=torch.float, device=device) + 99999999999999
            else:
                distance_mat = torch.zeros((a_p.shape[0], len(dataset.all_class_ids)), dtype=torch.float, device=device) + 99999999999999
                distance_mat_zsl = torch.zeros((a_p.shape[0], len(dataset.all_class_ids)), dtype=torch.float, device=device) + 99999999999999
            if mode == "audio":
                distance_mat[:, seen_unseen_array] = torch.cdist(a_p, classes_embeddings)
                ...
                distance_mat[:, seen_unseen_array] = torch.cdist(v_p, classes_embeddings)
                mask = torch.zeros(len(dataset.all_class_ids), dtype=torch.long, device=device)
                mask[seen_label_array] = 99999999999999
                distance_mat_zsl = distance_mat + mask
                if distance_fn == "SquaredL2Loss":
                ...
                audio_distance = torch.cdist(a_p, classes_embeddings, p=2)
                video_distance = torch.cdist(v_p, classes_embeddings, p=2)
            ...
            match_counts = torch.bincount(neighbor_batch[match_idx], minlength=len(dataset.all_class_ids))[seen_unseen_array]
